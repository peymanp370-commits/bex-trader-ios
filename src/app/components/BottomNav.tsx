...
export function BottomNav() {
  ...
  const navItems = [
    ...
    { to: "/app/account", label: tr(lang, "Account", "حساب", "الحساب"), icon: UserCircle },
      ...
      <div className="mx-auto flex max-w-md items-center justify-around px-2 py-2">
        {navItems.map((item) => {
          ...
          const active =
            ...
            (item.to === "/app/account" && location.pathname === "/app/my-stats");
          return (
            <Link
              key={item.to}
              to={item.to}
              className={`flex min-w-0 flex-1 flex-col items-center justify-center rounded-xl px-1 py-2 text-[11px] transition ${
