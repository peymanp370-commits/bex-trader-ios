...
import { Home, BarChart3, LineChart, Crown, Settings, Wrench } from "lucide-react";
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-gray-800 bg-[#0f1724]/95 backdrop-blur">
      <div className="mx-auto flex max-w-md items-center justify-around px-2 py-2">
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.to} to={item.to} className={`flex min-w-0 flex-1 flex-col items-center justify-center rounded-xl px-1 py-2 text-[11px] transition ${active ? "text-yellow-400" : "text-gray-400 hover:text-white"}`}>
